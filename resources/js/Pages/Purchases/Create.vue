...
//購入可能なアイテムをリスト化したい。数量を変更していくために、reactiveでデータを新規配列化しておくと便利。まず空のitemList配列を作成
const itemList = ref([])
...
//合計金額を求めるtotalPrice関数をcomputedで監視
...
//送信ボタンを押すことでControllerに送信される情報
const storePurchase = () => {
    //変数date,customer_id,statusはreactiveになっているので、追加処理を行わずともOK
    //一方でitemListは、現時点での数量が0以外のもののみ、pushでitems配列に格納してcontrollerに送るものとする。
    itemList.value.forEach(item => {
        if (item.quantity > 0) {
            form.items.push({
                id: item.id,
